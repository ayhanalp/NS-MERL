...
class TD3(object) :
	...
	def __init__(self, id, algo_name, state_dim, action_dim, hidden_size, actor_lr, critic_lr, gamma, tau, savetag,
				 foldername, actualize, use_gpu, init_w=True):
		...
		self.tracker = utils.Tracker(foldername, ['q_' + savetag, 'qloss_' + savetag, 'policy_loss_' + savetag,
												  'alz_score' + savetag, 'alz_policy' + savetag], '.csv',
		...
		if actualize:
			...
			if init_w:
				self.ANetwork.apply(utils.init_weights)
	...
	def update_parameters(self, state_batch, next_state_batch, action_batch, reward_batch, done_batch, global_reward,
		...
		if isinstance(state_batch, list):
			...
			reward_batch = torch.cat(reward_batch).done_batch = torch.cat(done_batch)
		...
		for _ in range(num_epoch) :
			...
			with torch.no_grad():
				...
				next_action_batch = self.policy_target.clean_action(next_state_batch,
																	return_only_action=True) + policy_noise.cuda() if self.use_gpu else policy_noise
			...
			if self.actualize:
				...
				target_Ascore = (self.actualize_lr) * (global_reward * 10.0) + (
							1 - self.actualize_lr) * current_Ascore.detach()
			...
			if self.algo_name == 'TD3' or self.algo_name == 'TD3_max':
			...
			if self.actualize:
				...
				if self.agent_id == 0:
					self.tracker.update(
						[self.q['mean'], self.q_loss['mean'], self.policy_loss['mean'], self.alz_score['mean'],
						 self.alz_policy['mean']], self.total_update)
...
class MultiTD3(object):
	...
	def update_parameters(self, state_batch, next_state_batch, action_batch, reward_batch, done_batch, global_reward, agent_id, num_epoch=1, **kwargs):
		"""Runs a step of Bellman update and policy gradient using a batch of experiences
		...
		for _ in range(num_epoch):
			...
			# print("Cross H", torch.nn.functional.cross_entropy(current_q1, target_q))
			# print("Cross H", torch.nn.functional.cross_entropy(self.policy, self.policy_target))
