...
from core.off_policy_algo import TD3, MultiTD3
...
Transition = namedtuple(
    "Transition",
    ("state", "next_state", "action", "reward", "done", "global_reward"),
)
...
class Agent :
    ...
    def __init__(self, args, id, adv=False) :
        self.args = args
        self.id = id

        # Initialize neuroevolution module###
        ...
        if args.ps == 'trunk' :
            if self.args.is_matd3 or args.is_maddpg :
                ...
                self.algo = MultiTD3(id, args.algo_name, args.state_dim, args.action_dim, args.hidden_size,
                                     args.actor_lr,
                                     args.critic_lr, args.gamma, args.tau, args.savetag, args.aux_save, args.actualize,
                                     args.use_gpu, args.config.num_agents, args.init_w)
            ...
            if args.algo_name == 'TD3' :
                self.algo = TD3(id, args.algo_name, args.state_dim, args.action_dim, args.hidden_size, args.actor_lr,
                                args.critic_lr, args.gamma, args.tau, args.savetag, args.aux_save, args.actualize,
                                args.use_gpu, args.init_w)
        ...
        # Initialize buffer
        if args.ps == 'trunk':
            self.buffer = [Buffer(args.buffer_size, buffer_gpu=False, filter_c=args.filter_c) for _ in
    ...
    def evolve(self) :
        ...
        if self.args.popn_size > 1 :  # If not no-evo
            ...
            if self.args.scheme == 'multipoint' :
                ...
                buffer_pointer = self.buffer[0] if self.args.ps == 'trunk' or self.args.ps == 'rtrunk' else self.buffer
    ...
    def update_rollout_actor(self) :
        for actor in self.rollout_actor :
            ...
            utils.hard_update(actor, self.algo.policy)
    ...
    def make_champ_team(self, agents) :
        for agent_id, agent in enumerate(agents) :
            if self.args.popn_size <= 1 :  # Testing without Evo
                ...
                utils.hard_update(self.rollout_actor[agent_id], agent.rollout_actor[0])

            else :
                utils.hard_update(self.rollout_actor[agent_id], agent.popn[agent.champ_ind])
